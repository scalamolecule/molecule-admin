package moleculeadmin.client.rxstuff
...
trait RxBindings {
  ...
  /**
    ...
    */
  implicit def htmltag2frag(r: Rx[HtmlTag])(implicit ctx: Ctx.Owner): Frag = bind(r,
  ...
  implicit def stylesheetcls2frag(r: Rx[Cls])(implicit ctx: Ctx.Owner): Frag = bind(r,
  ...
  private def bind[T](r: Rx[T], getSafeRxValue: Rx[T] => html.Element)(implicit ctx: Ctx.Owner): Frag = {
  ...
  implicit def observeAttrValue[T: AttrValue](implicit ctx: Ctx.Owner) = new AttrValue[Rx.Dynamic[T]] {
    def apply(t: Element, a: Attr, r: Rx.Dynamic[T]): Unit = {
      r.trigger {implicitly[AttrValue[T]].apply(t, a, r.now)} // observer
    }
  }

  implicit def observeStyleValue[T: StyleValue](implicit ctx: Ctx.Owner) = new StyleValue[Rx.Dynamic[T]] {
