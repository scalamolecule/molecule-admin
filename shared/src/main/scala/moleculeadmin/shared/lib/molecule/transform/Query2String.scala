package moleculeadmin.shared.lib.molecule.transform

import java.util.{Date, UUID}
import moleculeadmin.shared.lib.molecule.ast.query.{DS, Empty, ImplDS, In, NoBinding, NoVal, Query, QueryExpr, _}
import moleculeadmin.shared.lib.molecule.transform.exception.Query2StringException
...
case class Query2String(q: Query) extends Helpers {
  ...
  def p(expr: QueryExpr): String = expr match {
    ...
    case Val(v)                                           => "\"" + v + "\""
    case Pull(e, ns, attr, Some(prefix))                  => s"\n        (pull ?$e [{:$ns/$attr [:db/ident]}])"
    case Pull(e, ns, attr, _)                             => s"\n        (pull ?$e [:$ns/$attr])"
    case NoVal                                            => ""
    case DS(name)                                         => "$" + name
    case DS                                               => "$"
    case Empty                                            => ""
