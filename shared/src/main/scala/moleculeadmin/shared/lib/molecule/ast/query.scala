...
import moleculeadmin.shared.lib.molecule.transform
import moleculeadmin.shared.lib.molecule.util.Helpers

/** AST for molecule [[query.Query Query]] representation.
  * <br><br>
...
object query extends Helpers {
  ...
  /** Molecule Query representation.
    ...
    * Custom DSL molecule --> Model --> Query --> Datomic query string
    * <br><br>
    * Query is thus derived from [[model.Model Model]] representation.
    *
  ...
  case class Query(f: Find, wi: With, i: In, wh: Where) extends QueryExpr {
    lazy val print = transform.Query2String(this)
    ...
    def rules: String = if (i.rules.isEmpty) "none\n\n" else "[\n " + i.rules.map(transform.Query2String(this).p(_)).mkString("\n ") + "\n]"
    def debug: String =
    s"""$datalog
       |
       |RULES: $rules
...
//    override def toString: String = {
//      val v2 = if(v.toString.startsWith("__n__")) v.toString.drop(5).toDouble else v
//      s"""Val(${cast(v2)})"""
//    }
  }
