package moleculeadmin.shared.lib.molecule.ast

import java.util.Date
import moleculeadmin.shared.lib.molecule.ast.exception.ModelException
import moleculeadmin.shared.lib.moleculeExtras.HelpersAdmin
import moleculeadmin.shared.util.DateHandling


/** AST for molecule [[model.Model Model]] representation.
 * <br><br>
 * Molecule transforms custom boilerplate DSL constructs to Datomic queries in 3 steps:
 * <br><br>
 * Custom DSL molecule --> Model --> Query --> Datomic query string
 * */
object model extends HelpersAdmin with DateHandling {
  ...
  final def seq2[T](tpe: String, values: Seq[T]) = values.map {
  ...
  final protected def tv(tpe: String, value: Value) = value match {
    case VarValue                              => "VarValue"
    case Eq(Nil)                               => s"Eq(Seq())"
    case Eq((vs: Seq[_]) :: Nil) if vs.isEmpty => s"Eq(${seq2(tpe, vs)})"
    case Eq((vs: Seq[_]) :: Nil)               => s"Eq(${seq2(tpe, vs)})"
    case Eq(vs)                                => s"Eq(${seq2(tpe, vs)})"
    case Neq(vs)                               => s"Neq(${seq2(tpe, vs)})"
