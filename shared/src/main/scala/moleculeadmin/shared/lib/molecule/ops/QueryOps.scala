package moleculeadmin.shared.lib.molecule.ops

import java.net.URI
import java.util.{Collections, Date, UUID}
import moleculeadmin.shared.lib.molecule.ast.query._
//import datomic.Util
import moleculeadmin.shared.lib.molecule.ast.model._
import moleculeadmin.shared.lib.molecule.ast.query.{Funct, _}
import moleculeadmin.shared.lib.molecule.ops.exception.QueryOpsException
import moleculeadmin.shared.lib.molecule.util.Helpers


/** Query operations */
object QueryOps extends QueryOps {

}

trait QueryOps extends Helpers {
...
//      case "java.util.Date" => q.func(s"""ground #inst "${date2str(arg.asInstanceOf[Date])}"""", Empty, v)
    ...
    // Extra for molecule-admin ----------------------------------------------------------------------------------

    object Util {
      def list(items: AnyRef*) = {
        if (items == null) {
